...
class RETrainer(BaseTrainer):
    ...
    def test(self):
        ...
        tsne = TSNE(n_components=2, perplexity=20, learning_rate=200, n_iter=10000, random_state=42)
        colors = plt.cm.jet(np.linspace(0, 1, len(np.unique(all_labels))))
        all_sample_vectors_tsne = tsne.fit_transform(all_sample_vectors)
        plt.scatter(all_sample_vectors_tsne[:, 0], all_sample_vectors_tsne[:, 1], c=colors, cmap='jet')
        plt.savefig('plot_high.png')
