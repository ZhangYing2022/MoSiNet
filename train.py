    ...
    def test(self):
        ...
        with torch.no_grad():
            with tqdm(total=len(self.test_data), leave=False, dynamic_ncols=True) as pbar:
                ...
                total_loss = 0
                all_sample_vectors = np.empty((0, 1536))
        ...
        tsne = TSNE(n_components=2, perplexity=20, learning_rate=200, n_iter=900, random_state=15)
...
class NERTrainer(BaseTrainer):
    ...
    def _step(self, batch, mode="train"):
        if self.args.use_prompt:
            input_ids, token_type_ids, attention_mask, labels, images, aux_imgs, weight, input_ids_g, token_type_ids_g, attention_mask_g = batch
        else:
            images, aux_imgs = None, None
            input_ids, token_type_ids, attention_mask, labels = batch
        output = self.model(input_ids=input_ids, attention_mask=attention_mask,
                            token_type_ids=token_type_ids, labels=labels, images=images, aux_imgs=aux_imgs,
                            input_ids_phrase=input_ids_g,
                            weight = weight,
                            attention_mask_phrase=attention_mask_g
                            )
